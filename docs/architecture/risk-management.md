...
| 2 | **레버리지 한도** | **3x** (bybit.py MAX_LEVERAGE 상수 + SafetyGuard 이중 강제) | 명시적 레버리지 + 암묵적 레버리지(기존 포지션 합산) 이중 검증 |
...
### 3.1 Supertrend 4h Long-only 3x 전략

| 위험 항목 | 설정값 | 비고 |
|-----------|--------|------|
| 최대 레버리지 | 3x | SafetyGuard 하드캡 |
| 자본 배분 | 95% (×3x) | 100% 고정 배분 |
| min_notional | $65 | 최소 주문 금액 |
| ATR 손절·익절 | ATR(14)×3.3 | 4h 기준 |
| 안전 stop | 진입가 -70%/3x | 시장가 스탑 |
| Phase5 일일 절대 손실한도 | $10 AND 5% | trigger_mode: and |
...
- `BYBIT_TESTNET=false` -- Phase 5 메인넷 실전 중
- 초기 자본: EXPECTED_INITIAL_BALANCE_USD=185.31
...
### 6.4 거래소 스탑로스 주문 관리 (stoploss_on_exchange)
