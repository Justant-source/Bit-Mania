...
```mermaid
flowchart TD
    A["매수 신호 발생\nbase_amount: $100"] --> B["멀티플라이어 계산\n(Fear&Greed × EMA × RSI × ATR)"]
    ...
    E1 & E2 --> F{["Step 3: 주간 한도\n남음? $500"]}
    
    F -->|부족| F1["금액 재조정\nReason: weekly_cap"]
    F -->|충분| F2["진행"]
    
    F1 & F2 --> G{["Step 4: 총액 한도\n남음? $5,000"]}
    
    G -->|부족| G1["금액 재조정\nReason: total_deployed"]
    G -->|충분| G2["진행"]
    
    G1 & G2 --> H{["포지션 상태\n확인"]}
    
    H -->|"손실 -25% 이하"| H1["경고: 매수 중지\npause_on_drawdown"]
    H -->|"-15% 이상"| H2["OK: 매수 진행"]
