...
- **리스크**: MDD −73.29% (극단 시나리오 대비 고위험, 사용자 승인 필수)
...
|------|-----|------|
| **CAGR** | +137.64% | ✅ 매우 우수 |
| **Sharpe Ratio** | 1.349 | ⚠️ 1.0 이상 (적절) |
...
### Phase 5 오버라이드 (메인넷 소액 실전 $185.31 USDT)

```yaml
phase5:
  sizing_mode: fixed_notional
  fixed_notional_usd: 150              # 할당 자본의 95% × 3x
  max_concurrent_positions: 1          # 동시 1개만
  min_position_usd: 65                 # Bybit 최소 주문
  expected_initial_balance_usd: 185.31 # Phase 5 시작 잔고
```
...
초기:
  - 자본: $185.31 USDT
  - Supertrend 배분: 100% (단일 전략)
  - 포지션 배분: 95% × 3x = $527.40 명목가
  - BTC 가격: $60,000
  - 포지션: ~0.0088 BTC (3x)

상승: BTC → $78,000 (+30%)
  - Long 이익: 0.0088 × (78,000 - 60,000) × 3 = $4,752
  - 수수료: -$35
  - 순 P&L: +$4,717
  - 최종 equity: $4,902
```
...
하락: BTC → $30,000 (-50%)
  - Long 손실: 0.0088 × (30,000 - 60,000) × 3 = -$792
  - 수수료: -$20
  - 순 P&L: -$812
  - 최종 equity: -$626.69 (마진 콜)
  
→ Kill Switch 발동 (절대값 기준 -$10 이상 AND -5%)
→ 모든 포지션 즉시 청산
```
