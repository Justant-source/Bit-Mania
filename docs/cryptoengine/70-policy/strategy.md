...
| 심볼 | BTCUSDT — 라이브는 Bybit USDT 무기한 4h, 백테스트 정본 시세는 Binance **현물** 4h (§6 출처 정정 참조) |
| 타임프레임 | 4h |
| 방향 | Long-only (숏 없음) |
...
## §6. 백테스트 성과 (Binance **현물** 4h·00계열 격자 정본)

> ⚠️ **데이터 출처 정정 (2026-08-31)**: 이 절은 오랫동안 "Bybit 네이티브 4h 정본"으로 기재돼 있었으나, 조사 결과 **정본 백테스트의 시세는 Binance 현물(spot)** 이다(픽스처 `btc_4h*.csv` ← `jesse_db.ohlcv_4h` ← Binance Vision spot; `exchange` 컬럼의 `'Bybit Perpetual'`은 다운로드 스크립트가 하드코딩한 라벨이었고 2026-08-31 `'Binance Spot'`으로 정정). 검증: 2020-03-12·2022-06-15·2026-08-28 각 1,440봉에서 **spot 100% 일치, perp 0%**. 라이브는 **Bybit USDT 무기한**이라 거래소도 상품 종류도 다르며 종가 기준 평균 **+0.05%** 차이가 난다. 2026-06-14 정합 작업이 고친 **격자 어긋남(02계열→00계열)** 수정 자체는 유효하다 — 틀린 것은 출처 라벨뿐이다.
>
> 아래 지표는 그대로 유효하되 **"Binance 현물 시세 기준, 펀딩비 미반영"으로 읽어야 한다.** ① 동일 구간을 라이브 Bybit 데이터로 재실행하면 net +3.1%p·MDD +3.2%p 차이(34거래 중 32건 동일)이고, ② **무기한의 펀딩비는 현물 캔들에 존재하지 않아 구조적으로 0으로 가정**돼 있다(롱 보유 시 일반적으로 지불 쪽, 3x라 명목가 기준 3배 — 미정량, 별도 과제). 상세: `backtest/results/2026-08-31/csv_ohlcv_drift.md`.
...
- 4h 00계열 격자 1:1 정본 (시세 출처는 Binance 현물 — §6 출처 정정 참조)
...
이 비용모델로 v12 1,536-combo 격자(frozen, 재사전등록 아님)도 재채점했다: `#7908`을 MDD·CAGR 동시 지배하는 combo는 여전히 0개지만, `dir_ema=220&fast_ema=4` 방향에서 plateau_pass가 0→7로 늘고 그중 6개가 `#7908`보다 높은 S_raw를 보였다 — 채택하지 않았고(사전등록 없는 사후 관찰), v13 독립 사전등록 후보로만 기록. 상세: `backtest/results/v12/VERDICT.md` "후속" §.
