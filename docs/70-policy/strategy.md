...
⚠️ **정합감사 중 별도의 라이브 이상 발견**: 2026-08-19 청산이 `exit_reason='atr_distance'`로 기록됐으나, 그 조건이 성립하려면 진입가가 실제 체결가(64,153.5)보다 **$6,876 높은 ~71,030 이상**이어야 함을 확인 — 반올림/정밀도로 설명 불가, 다른 체결 기록도 없음. `strategy.py`의 `_sync_position_from_exchange()`가 거래소(Bybit) 보고 진입가를 그대로 신뢰하는 구조상, **그 시점 거래소 보고값 자체의 이상**으로 원인이 좁혀지나 로그 보존기간 만료로 확정 불가. 이번 사례는 유리한 방향(조기이지만 수익 청산)으로 작동해 손실은 없었으나, **반대 방향으로도 작동할 수 있어 별도 조사가 필요하다** — 코드에 진입가 드리프트 로깅이 없음(수량 드리프트만 로깅)도 함께 확인. 라이브 코드는 이 세션에서 수정하지 않았다. 상세: `backtest/results/2026-08-31/live_slippage_report.md` §5.1.
