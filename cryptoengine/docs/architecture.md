...
CryptoEngine은 비트코인 선물 자동매매를 위한 마이크로서비스 기반 시스템입니다. 시장 레짐 감지, 전략 오케스트레이션, 리스크 관리, LLM 기반 어드바이저를 통합하여 24/7 무인 운영이 가능하도록 설계되었습니다.
...
- **기능**:
  ...
  - Kill Switch 조건 검사
  - LLM 어드바이저 가중치 조정 통합
- **출력**: Redis를 통해 각 전략에 자본 배분 명령 전달

### 3. Execution Engine (`services/execution/`)
- **역할**: 주문 수신, 검증, 실행, 결과 배포
- **기능**:
  - Redis `order:request` 채널 구독
  - 안전성 검사 (Safety Guard)
  - 동시 주문 제한 (세마포어)
  - 재시도 로직 (최대 3회, 지수 백오프)
  - 멱등성 보장 (request_id 기반)
- **출력**: `order:result` 채널로 결과 배포, DB 저장

### 4. Trading Strategies

#### 4a. Funding Rate Arbitrage (`services/strategies/funding-arb/`)
- 델타 중립 포지션: 현물 매수 + 무기한 선물 매도
- 펀딩비 수취 목적
- Basis Spread State Machine으로 진입/청산 결정
- 한쪽 체결 복구 로직 (3분 대기)

#### 4b. Grid Trading (`services/strategies/grid-trading/`)
- 횡보장에서 그리드 주문 배치
- 기하급수/등차 그리드 지원
- 변동성 기반 자동 활성화/비활성화
- ATR 기반 그리드 범위 설정

#### 4c. Adaptive DCA (`services/strategies/adaptive-dca/`)
- Fear & Greed Index 기반 매수량 조절
