...
```mermaid
flowchart TD
    START["Jesse 백테스트 완료<br/>3년 실데이터: 2023-04~2026-04"] --> CAGR{"Jesse 3yr<br/>CAGR 확인"}
    
    CAGR -->|≥ 13%| S1["시나리오 A: 동등성 확인<br/>자체엔진과 유사 성과"]
    CAGR -->|8-12%| S2["시나리오 B: 간격 발생<br/>합성폴백 영향 추정 3-5%p"]
    CAGR -->|< 8%| S3["시나리오 C: 큰 차이<br/>자체엔진 로직 감사 필요"]
    
    S1 --> A1["결론: 자체엔진 로직 정확<br/>fa80_lev5_r30 유지 가능"]
    A1 --> A2["의사결정: ✅ PASS<br/>Phase 5 메인넷 진입 준비 완료"]
    
    S2 --> B1["결론: 2020-2023년<br/>과장 정도 3-5%p"]
    B1 --> B2["파라미터 보수화<br/>fa80_lev4_r30 검토<br/>또는 fa60_lev5_r30"]
    B2 --> B3["의사결정: ⚠️ CONDITIONAL PASS<br/>기대값 조정 + 6개월 강화모니터링"]
    
    S3 --> C1["결론: 자체엔진 버그<br/>가능성 높음"]
    C1 --> C2["즉시 자체엔진<br/>코드 감사 실시"]
    C2 --> C3["의사결정: 🔍 INVESTIGATION<br/>메인넷 진입 보류"]
