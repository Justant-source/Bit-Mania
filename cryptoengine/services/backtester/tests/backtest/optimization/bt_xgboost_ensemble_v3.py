...
Execution:
    docker compose --profile backtest run --rm backtester \
      python tests/backtest/optimization/bt_xgboost_ensemble_v3.py --stage 1
"""
...
class WalkForwardBacktester:
    """Walk-Forward XGBoost ensemble backtester with Purged CV (v3)."""
    ...
    async def run(
        ...
        """Run walk-forward backtest with Purged CV (v3)."""
        if not XGBOOST_AVAILABLE:
            return {"error": "xgboost not installed"}

        log.info("backtest_start v3_purged_cv", windows=len(self.windows), threshold=prob_threshold)
        ...
        for window in self.windows:
            ...
            # Get indices (with purge/embargo)
        ...
        agg_sharpe = np.mean(sharpe_list)
        n_folds = len(fold_results)

        # v3: Compute Deflated Sharpe Ratio
        dsr = deflated_sharpe_ratio(agg_sharpe, n_trials=n_folds, obs_per_year=1461)

        result = {
            "n_folds": n_folds,
            "agg_oos_sharpe": safe_float(agg_sharpe),
            ...
            "deflated_sharpe_ratio": safe_float(dsr),  # v3: Deflated SR (0~1, >0.95 strong)
        ...
        log.info(
            "backtest_complete_v3",
            agg_sharpe=f"{agg_sharpe:.3f}",
            dsr=f"{dsr:.3f}",
            n_folds=n_folds,
        )
...
async def run_stage_1_baseline(wf_bt: WalkForwardBacktester) -> dict[str, Any]:
    """Stage 1: Baseline (v3 Purged CV)."""
    log.info("stage_1_baseline v3_purged_cv starting")
...
async def main(args):
    ...
    if "error" not in result_s1:
        ...
        print(f"  Folds: {result_s1['n_folds']}")
        print(f"  Agg Sharpe: {result_s1['agg_oos_sharpe']:.3f}")
        print(f"  Deflated Sharpe: {result_s1['deflated_sharpe_ratio']:.3f}")
        print(f"  CAGR: {result_s1['agg_oos_cagr']:.2f}%")
        print(f"  MDD: {result_s1['agg_oos_mdd']:.2f}%")
        print(f"  Trades: {result_s1['total_trades']}")

        await save_result(pool, "stage_1_baseline", "xgboost_v3_purged_cv",
                        {"oos_sharpe": result_s1["agg_oos_sharpe"],
                         "deflated_sharpe": result_s1["deflated_sharpe_ratio"],
                         "oos_cagr": result_s1["agg_oos_cagr"],
                         "oos_mdd": result_s1["agg_oos_mdd"]},
                        result_s1["hyperparams"],
                        "xgboost_ensemble_v3_results")
    else:
        print(f"Stage 1 Error: {result_s1['error']}")

    await pool.close()
    print("\nBacktest v3 (Purged CV + Deflated Sharpe) Complete")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XGBoost Ensemble v3 — CPCV + Deflated Sharpe")
    parser.add_argument("--stage", choices=["1", "2", "3", "4", "5", "all"], default="1", help="Stage to run")
