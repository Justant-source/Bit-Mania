#!/usr/bin/env python3
"""
v6_worker.py — v6 15×15×8 = 1,800 (combo,window) SQLite claim-and-run 워커

v5_worker.py와 거의 동일. 변경점:
  - claim_job: tp_atr_mult, sl_atr_mult, v5_atr_mult 컬럼 추가
  - build_hp_json: atr_mult(v5_atr_mult) + tp_atr_mult + sl_atr_mult 포함
  - --strategy SupertrendStrategyAsymmetric

Usage (v6_master.py 내부에서 호출):
    python3 /app/scripts/optimization/v6_worker.py \
...
DB_TIMEOUT       = 30
BACKTEST_TIMEOUT = 600  # 10 min per window
...
def claim_job(conn: sqlite3.Connection) -> dict | None:
    ...
    try:
        row = conn.execute(
            "SELECT job_id, combo_id, carrier_id, v5_combo_id, window_name, w_start, w_end, "
            "st_factor, st_period, fast_ema_len, slow_ema_len, "
            "direction_ema_len, tp_atr_mult, sl_atr_mult, sl_margin_pct, "
            "v5_atr_mult, v5_mean_cagr, v5_worst_mdd FROM jobs "
        ...
        conn.execute("ROLLBACK")
        raise
    keys = ['job_id', 'combo_id', 'carrier_id', 'v5_combo_id', 'window_name', 'w_start', 'w_end',
            'st_factor', 'st_period', 'fast_ema_len', 'slow_ema_len',
            'direction_ema_len', 'tp_atr_mult', 'sl_atr_mult', 'sl_margin_pct',
...
def build_hp_json(job: dict) -> str:
    hp = {
        ...
        'atr_mult':          job['v5_atr_mult'],   # base class hp — unused by asymmetric update_position
        'tp_atr_mult':       job['tp_atr_mult'],
        'sl_atr_mult':       job['sl_atr_mult'],
        'sl_margin_pct':     job['sl_margin_pct'],
    }
...
def run_backtest(job: dict, output_dir: Path) -> bool:
    ...
    run_dir = Path(tempfile.mkdtemp(prefix=f'jesse_v6c{combo_id}{w_name}_'))
...
def main():
    p = argparse.ArgumentParser(description='v6 (combo,window) worker')
    ...
    while True:
        ...
        if success:
            ...
            if stats:
                ...
                print(f"[c={combo_id} tp={job['tp_atr_mult']}/sl={job['sl_atr_mult']} {w_name}] "
                      f"done cagr={stats['cagr']:.1f}% mdd={stats['mdd']:.1f}% "
                      f"trades={stats['trades']} elapsed={elapsed:.0f}s", flush=True)
            else:
