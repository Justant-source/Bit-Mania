#!/usr/bin/env python3
"""
v4_master.py — v4 216 combo × 8 window = 1,728 job 스윕 마스터 (SQLite 큐)

1. Jesse 환경 부트스트랩
2. v4_generate_grid.py 호출 (idempotent)
3. ProcessPoolExecutor(workers=N)로 v4_worker.py spawn
4. 30초 주기 진행 모니터링
5. 완료 후 v4_aggregate.py 자동 호출

Usage (inside Jesse container):
    python3 /app/scripts/optimization/v4_master.py --workers 6
    python3 /app/scripts/optimization/v4_master.py --workers 4 --skip-post
"""
...
DB_PATH    = '/result/v4_optimization/queue.sqlite3'
OUTPUT_DIR = '/result/v4_optimization'
TOTAL      = 1728  # 216 combos × 8 windows
...
def generate_grid() -> None:
    ...
    if result.returncode != 0:
        print('[ERROR] v4_generate_grid.py failed', file=sys.stderr)
        sys.exit(1)
...
def run_post_processing() -> None:
    print('\nRunning v4_aggregate.py...', flush=True)
    subprocess.run(
...
def main():
    p = argparse.ArgumentParser(description='v4 sweet-spot master (1,728 jobs)')
    p.add_argument('--workers', type=int, default=6)
    p.add_argument('--skip-post', action='store_true', help='Skip v4_aggregate.py')
    args = p.parse_args()

    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

    print(f'v4 Master — {TOTAL} jobs (216 combos × 8 windows), {args.workers} workers')
    print(f'DB: {DB_PATH}')
