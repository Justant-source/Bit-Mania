#!/usr/bin/env python3
"""
v5_2_worker.py — v5_2 1,296×8 = 10,368 (combo,window) SQLite claim-and-run 워커

v5_worker.py와 거의 동일. 변경점:
  - WAL pragma 10-retry backoff 루프 (6-worker 동시기동 race condition 해결, v6 검증됨)
  - 경로 prefix: jesse_v5_2c

Strategy: SupertrendStrategyWithSL (v5와 동일, 대칭 atr_mult, sl_margin_pct=0.0)

Usage (v5_2_master.py 내부에서 호출):
...
def run_backtest(job: dict, output_dir: Path) -> bool:
    ...
    run_dir = Path(tempfile.mkdtemp(prefix=f'jesse_v5_2c{combo_id}{w_name}_'))
...
def main():
    p = argparse.ArgumentParser(description='v5_2 (combo,window) worker')
    ...
    # WAL retry loop (v6 버그픽스): 6 workers 동시기동 시 race condition 방지
