#!/usr/bin/env python3
"""
v5_worker.py — v5 324×8 = 2,592 (combo,window) SQLite claim-and-run 워커

v4_worker.py와 완전 동일 (generic — DB 경로는 --db 인자로 지정).
sl_margin_pct=0.0 고정 (v5 grid 동일 조건).

Usage (v5_master.py 내부에서 호출):
...
def run_backtest(job: dict, output_dir: Path) -> bool:
    ...
    run_dir = Path(tempfile.mkdtemp(prefix=f'jesse_v5c{combo_id}{w_name}_'))
...
def main():
    p = argparse.ArgumentParser(description='v5 (combo,window) worker')
