#!/usr/bin/env python3
"""
v6_master.py — v6 225 combo × 8 window = 1,800 job 스윕 마스터 (SQLite 큐)

v5_master.py와 동일 패턴. 그리드/경로만 v6로 변경.

Usage (inside Jesse container):
    python3 /app/scripts/optimization/v6_master.py --workers 6
    python3 /app/scripts/optimization/v6_master.py --workers 4 --skip-post
"""
...
DB_PATH    = '/result/v6_optimization/queue.sqlite3'
OUTPUT_DIR = '/result/v6_optimization'
TOTAL      = 1800  # 225 combos × 8 windows
...
def generate_grid() -> None:
    ...
    if result.returncode != 0:
        print('[ERROR] v6_generate_grid.py failed', file=sys.stderr)
        sys.exit(1)
...
def run_post_processing() -> None:
    print('\nRunning v6_aggregate.py...', flush=True)
    subprocess.run(
...
def main():
    p = argparse.ArgumentParser(description='v6 asymmetric TP/SL master (1,800 jobs)')
    p.add_argument('--workers', type=int, default=6)
    p.add_argument('--skip-post', action='store_true', help='Skip v6_aggregate.py')
    args = p.parse_args()

    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

    print(f'v6 Master — {TOTAL} jobs (225 combos × 8 windows), {args.workers} workers')
    print(f'DB: {DB_PATH}')
