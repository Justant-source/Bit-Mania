"""Migrate v5_2 optimization results to database."""
import csv
import json
...
def main():
    """Load v5_2 optimization CSV and populate database."""
    ...
    upsert_sweep(
        conn,
        sweep_id='v5_2',
        description='v5_2 dense grid (1296 combos)',
        leverage=3,
        variant='long_only',
        grid_json=None,
        n_combos=1296,
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            combo_data = {
                ...
                'worst_mdd_recent': _f(row, 'worst_mdd_recent'),
                'mean_cagr_recent': _f(row, 'mean_cagr_recent'),
                'tier1': _b(row, 'tier1'),
                'tier2': _b(row, 'tier2'),
                'tier3': _b(row, 'tier3'),
                'tier4': _b(row, 'tier4'),
                ...
                'cross_val_status': row.get('cross_val_status'),
                'xref_json': json.dumps(xref),
                'raw_json': json.dumps(dict(row))
            }

            pk = upsert_combo(conn, 'v5_2', combo_data)
    ...
    conn.close()
    print(f'v5_2 migration complete: {row_count} combos loaded')
