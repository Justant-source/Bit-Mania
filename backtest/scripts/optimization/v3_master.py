#!/usr/bin/env python3
"""
v3_master.py — v3 fine-grid 1,620 combo 스윕 마스터 (6 workers, SQLite 큐)

1. Jesse 환경 부트스트랩
2. v3_generate_grid.py 호출 (idempotent)
3. ProcessPoolExecutor(workers=6)로 v3_worker.py spawn
4. 30초 주기 진행 모니터링
5. 완료 후 v3_aggregate.py 자동 호출

Usage (inside Jesse container):
    python3 /app/scripts/optimization/v3_master.py --workers 6
    python3 /app/scripts/optimization/v3_master.py --workers 4 --skip-post
"""
...
DB_PATH = '/result/v3_optimization/queue.sqlite3'
OUTPUT_DIR = '/result/v3_optimization'
TOTAL = 1620
...
def generate_grid() -> None:
    result = subprocess.run(
        ['python3', '/app/scripts/optimization/v3_generate_grid.py',
         '--db', DB_PATH],
        cwd='/app', capture_output=False
    )
    if result.returncode != 0:
        print('[ERROR] v3_generate_grid.py failed', file=sys.stderr)
        sys.exit(1)
...
def run_worker(_args: tuple) -> int:
    ...
    result = subprocess.run(
        ['python3', '/app/scripts/optimization/v3_worker.py',
         '--db', db_path,
...
def run_post_processing() -> None:
    print('\nRunning v3_aggregate.py...', flush=True)
    subprocess.run(
...
def main():
    p = argparse.ArgumentParser(description='v3 fine-grid master (1,620 combos)')
    p.add_argument('--workers', type=int, default=6)
    p.add_argument('--skip-post', action='store_true', help='Skip v3_aggregate.py post-processing')
    args = p.parse_args()

    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

    print(f'v3 Master — {TOTAL} combos, {args.workers} workers')
    print(f'DB: {DB_PATH}')
