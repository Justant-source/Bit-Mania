...
def get_progress(sweep_id: str) -> tuple[int, int]:
    """(completed_combos, total_combos) 반환."""
    sql = (
        f"SELECT "
        f"  (SELECT COUNT(*) FROM st_combos WHERE sweep_id='{sweep_id}' AND n_complete=8), "
        f"  (SELECT COUNT(*) FROM st_combos WHERE sweep_id='{sweep_id}');"
    )
    result = subprocess.run(
        ['docker', 'exec', 'cryptoengine-backtest-postgres',
...
def start_master(sweep_id: str, workers: int) -> subprocess.Popen:
    cmd = [
        ...
        '--sweep', sweep_id,
        '--workers', str(workers),
        '--no-aggregate',   # aggregate는 최종 완료 시에만 실행
    ]
...
def main() -> int:
    ...
    while True:
        ...
        next_time = datetime.datetime.now() + datetime.timedelta(seconds=wait_s)

        done, total = get_progress(sweep_id)
        remaining   = total - done
        print(f'\n[scheduler] iter={iteration}  workers={workers}  '
              f'progress={done}/{total}  remaining={remaining}  '
              f'next_transition={next_time.strftime("%m/%d %H:%M")}',
              flush=True)
        ...
        while elapsed < wait_s:
            ...
            if proc.poll() is not None:
                # pg_master가 자연 종료 (모든 jobs 완료)
                completed = True
                break

        if completed:
            print('[scheduler] pg_master exited naturally. All jobs done. Running aggregate...',
                  flush=True)
            run_aggregate(sweep_id)
            print('[scheduler] Done.', flush=True)
            return 0
        ...
        print(f'[scheduler] Time transition at {now_str}. '
              f'Switching from {workers}w → {new_w}w. Stopping pg_master...',
              flush=True)
        ...
        if deleted:
            print(f'[scheduler] Cleaned up {deleted} stale placeholder rows.', flush=True)
