#!/usr/bin/env python3
"""
v5_master.py — v5 324 combo × 8 window = 2,592 job 스윕 마스터 (SQLite 큐)

v4_master.py와 동일 패턴. 그리드/경로만 v5로 변경.

Usage (inside Jesse container):
    python3 /app/scripts/optimization/v5_master.py --workers 6
    python3 /app/scripts/optimization/v5_master.py --workers 4 --skip-post
"""
...
DB_PATH    = '/result/v5_optimization/queue.sqlite3'
OUTPUT_DIR = '/result/v5_optimization'
TOTAL      = 2592  # 324 combos × 8 windows
...
def generate_grid() -> None:
    result = subprocess.run(
        ['python3', '/app/scripts/optimization/v5_generate_grid.py',
         '--db', DB_PATH],
        cwd='/app', capture_output=False
    )
    if result.returncode != 0:
        print('[ERROR] v5_generate_grid.py failed', file=sys.stderr)
        sys.exit(1)
...
def run_worker(_args: tuple) -> int:
    ...
    result = subprocess.run(
        ['python3', '/app/scripts/optimization/v5_worker.py',
         '--db', db_path,
...
def run_post_processing() -> None:
    print('\nRunning v5_aggregate.py...', flush=True)
    subprocess.run(
...
def main():
    p = argparse.ArgumentParser(description='v5 sweet-spot master (2,592 jobs)')
    p.add_argument('--workers', type=int, default=6)
    p.add_argument('--skip-post', action='store_true', help='Skip v5_aggregate.py')
    args = p.parse_args()

    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

    print(f'v5 Master — {TOTAL} jobs (324 combos × 8 windows), {args.workers} workers')
    print(f'DB: {DB_PATH}')
