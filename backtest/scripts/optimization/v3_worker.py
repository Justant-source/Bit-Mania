#!/usr/bin/env python3
"""
v3_worker.py — v3 fine-grid 1,620 combo SQLite claim-and-run 워커

v2 p1_worker.py와 동일한 패턴. DB/output 경로와 그리드만 다름.
sl_margin_pct=-25.0 고정 (DB에 저장된 값을 hp-json에 포함).

Usage (p3_master_v3.py 내부에서 호출):
...
def run_backtest(job: dict, output_dir: Path) -> bool:
    ...
    run_dir = Path(tempfile.mkdtemp(prefix=f'jesse_v3c{combo_id}_'))
...
def main():
    p = argparse.ArgumentParser(description='v3 fine-grid worker')
