#!/usr/bin/env python3
"""
Run Existing V4 Audit Scripts

Wrapper that runs existing v4 audit scripts and combines results into one JSON.

Scripts run:
  - audit_v4_data.py
  - sanity_check.py

Output: results/audit/01_existing_v4.json
...
def main():
    ...
    AUDIT_SCRIPTS_DIR = BT_ROOT / "scripts" / "audit"
    RESULTS_DIR = BT_ROOT / "results" / "audit"
    OUTPUT_FILE = RESULTS_DIR / "01_existing_v4.json"

    # Ensure output dir exists
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    # List of v4 audit scripts to run
    # Note: sanity_check.py requires per-job --result-json/--strategy args; not a standalone audit
    scripts_to_run = [
        ("audit_v4_data.py", AUDIT_SCRIPTS_DIR / "audit_v4_data.py", []),
    ]

    print("Running v4 audit scripts...", file=sys.stderr)
    ...
    # Build output JSON
    output = {
        "audit_id": "01_existing_v4",
        "name": "existing_v4_audit_wrapper",
