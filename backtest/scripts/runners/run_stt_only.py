#!/usr/bin/env python3
"""SupertrendTrendTypeStrategy 11개 변형만 실행 (멀티에이전트 분할용)."""
...
HP_STT   = json.dumps({'atr_len': 14, 'atr_ma_len': 20, 'di_len': 14, 'smooth': 1,
                       'st_factor': 3.0, 'st_period': 7, 'fast_ema_len': 7,
                       'slow_ema_len': 20, 'direction_ema_len': 200, 'atr_mult': 3.0})
HP_STT_V2 = json.dumps({'atr_len': 14, 'atr_ma_len': 20, 'di_len': 14, 'smooth': 1,
...
def main():
    print(f'run_stt_only: {len(JOBS)} jobs, {PARALLELISM} workers', flush=True)
    t_total = time.time()
    results = []
