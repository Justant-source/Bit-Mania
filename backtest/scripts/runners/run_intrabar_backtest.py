...
# DB helpers (ohlcv_1m / ohlcv_4h tables)
sys.path.insert(0, str(SCRIPTS_DIR.parent / 'db'))
...
def _load_1m(start: str, end: str) -> np.ndarray:
    """Load 1m OHLCV from PostgreSQL ohlcv_1m table.
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM ohlcv_1m "
                "WHERE symbol = 'BTCUSDT' AND timestamp >= %s AND timestamp < %s "
                "ORDER BY timestamp",
                (start_ms, end_ms),
            )
            rows = cur.fetchall()
    ...
    if not rows:
        raise FileNotFoundError(f'No 1m candles in ohlcv_1m for {start}~{end}')
...
def _load_1h(start: str, end: str) -> np.ndarray:
    """Load 1h OHLCV by aggregating ohlcv_1m from PostgreSQL.
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT (timestamp / 3600000)::bigint * 3600000 AS hour_ms, "
    ...
    if not rows:
        raise FileNotFoundError(f'No candles in ohlcv_1m for {start}~{end}')
