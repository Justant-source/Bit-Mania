...
| 파라미터 | 값 | 설명 |
|----------|-----|------|
| `min_funding_rate_annualized` | 15% | 최소 연환산 펀딩비 |
...
| 레짐 | 펀딩 차익 | 그리드 | DCA | 현금 |
|------|----------|--------|-----|------|
| **횡보 (Ranging)** | 0.25 | **0.40** | 0.15 | 0.20 |
| **상승 추세 (Trending Up)** | 0.15 | 0.05 | **0.50** | 0.30 |
| **하락 추세 (Trending Down)** | 0.20 | 0.05 | 0.10 | **0.65** |
| **고변동 (Volatile)** | 0.10 | 0.05 | 0.05 | **0.80** |

**핵심 원칙**:
- 횡보장: 그리드 트레이딩 최대 가동 (0.40)
- 상승장: DCA 적극 매수 (0.50)
- 하락장: 현금 비중 극대화 (0.65), 펀딩비 기회 포착 유지 (0.20)
- 고변동: 방어 모드, 현금 80%
...
- 또는 다중 타임프레임 피처를 결합하여 단일 타임프레임 의존도 감소
- Phase 3 백테스트 시 6개월 히스토리 데이터를 활용하여 모델 사전 학습 권장

### 6.4 기타 고려사항

- **펀딩비 전략 현물 레그**: 현재 Bybit 내에서 현물+선물을 동시 운용하나, 현물 유동성이 낮은 쌍(SOL, XRP)은 슬리피지 리스크가 높다. 쌍별 유동성 모니터링 추가 필요
- **그리드 트레이딩 트레일링**: `trailing_enabled: false`로 설정되어 있다. 횡보에서 미세 추세 전환 시 수익 기회를 놓칠 수 있으므로, 시뮬레이션 후 활성화 검토
- **DCA 복합 승수 상한**: 모든 승수 결합 시 최대 5.0x까지 허용되나, 극단적 시장에서 단일 매수가 500 USD(= 100 x 5.0)에 달할 수 있다. 일일/주간 한도와 함께 모니터링 필요
