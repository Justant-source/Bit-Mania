...
| AUX (비활성) | 그리드 트레이딩 | 6년 백테스트 전 변형 음수 → 가중치 0 고정 |
| AUX (비활성) | 적응형 DCA | 6년 WFO MDD 42% → 가중치 0, 재설계 필요 |
...
| 파라미터 | 기본값 | Phase 5 값 |
|----------|--------|-----------|
| 사이징 모드 | `pct_equity` (자기자본 비율) | `fixed_notional` ($150 고정) |
| 포지션당 비율 | 5% | — |
| 고정 명목가 | — | 150 USD ($200 × 75% 안전 버퍼) |
| 포지션당 상한 | 10,000 USD | — |
| 최소 포지션 | 100 USD | 50 USD |
| 최대 레버리지 | 5배 (fa80_lev5_r30 채택) | 5배 |
| 동시 포지션 수 | 최대 5개 | 최대 1개 |
| 헤지 비율 | 1.0 (완전 헤지) | 1.0 |
| 헤지 드리프트 허용 | 2% | 2% |

**Phase 5 자동 활성화**: `PHASE5_MODE=true` 또는 `BYBIT_TESTNET=false` 환경변수 감지 시 `config/strategies/funding-arb.yaml`의 `phase5:` 섹션 값 자동 적용.
...
> **on_start() 초기화**: `set_margin_mode("isolated")` + `set_leverage(5)` 자동 설정 후 거래 시작 (fa80_lev5_r30).

### 리스크 관리

- **포트폴리오 최대 배분**: 80% (fa_capital_ratio: 0.80)
- **최대 낙폭(MDD)**: 5% (7일 윈도우)
- **전략 상관계수 제한**: 0.7
...
├── strategy.py         # FundingArbStrategy(BaseStrategy) 구현 (Phase 5: fixed_notional 사이징)
├── funding_tracker.py  # 펀딩비 추적, 연환산 계산, 연속 간격 검증
│                       # + estimate_net_profit_per_cycle() — 수수료/슬리피지 반영 순수익/BEP 계산
│                       # + is_entry_net_profitable() — BEP <= 2회 여부 검증
...
| 트리거 | 임계값 (기본) | Phase 5 임계값 | 동작 |
|--------|-------------|---------------|------|
| 일일 낙폭 | **5%** | **5%** AND **$10** (AND 조건) | 전략 정지 + 포지션 청산 |
| 주간 낙폭 | **10%** | **10%** AND **$20** | 전략 정지 + 포지션 청산 |
| 월간 낙폭 | **15%** | **15%** AND **$30** | 전략 정지 + 포지션 청산 |
| 일일 손실 | 1,000 USD | — (절대값으로 대체) | 전략 정지 + 포지션 청산 |
...
- 최소 신뢰도: 0.5
- 최대 가중치 조정: +/-5%p (v1.5.0에서 15%p → 5%p 축소, AI 환각 시 오차 최소화)
